...
- 下一步候选：跨客户端线程锁争用（ChatGPT 桌面版占锁导致 `codex_send/steer/interrupt` 对旧线程失败，见决策 18）；错误信息透传上游原文收敛；config.toml 增量覆写实现（当前零代码读取）；插件命令在 GUI 子进程 PATH 中的可行性（开源一键安装需保证 `codex-as-subagent` 可被 Host 发现，或改用 `node` + 解析后的绝对路径，且 ZCode 插件 update 会从源目录重同步覆盖本地化 patch）；`doctor` 命令（设计 6.4 提及，未实现）；`npm run lint` 覆盖面（当前仅单文件语法检查）与 `tests/e2e`/`tests/fixtures` 空占位；session 级隔离（V2，设计 6.2 末尾）。
...
18. 跨客户端线程锁与错误透传（2026-09-11 实测发现，尚未修复，记为 V1 已知限制）。向"上一 Server 实例创建、当前实例未持有"的线程调 `codex_send` 时，上游返回 `thread ... already has an active writer`。
    - **根因**：Codex app-server 对每个 thread 用 flock 型写锁（`~/.codex/thread-writer-locks/<threadId>.lock`，二进制含 `writer_lock.rs`、"failed to acquire thread writer lock"）。`~/.codex` 跨客户端共享，同一账号下运行的 ChatGPT 桌面版 app-server 会持有这些线程的锁。实测：桌面版 PID 63149 同时持有 9 个历史线程的锁；本 Server 自己新建的线程由其自己的 app-server 持有，`send` 正常；Server idle 退出后其 app-server 释放锁，再次冷启动时旧线程被桌面版接管 → 故"换实例后对旧线程 send"最易触发。这是共享 Codex 存储的固有限制，非本项目锁实现缺陷；V1 隔离边界是 workspace，不含"Codex 客户端独占"维度。
    - **附带问题**：`asDomainError`（`src/core/runtime-manager.mjs`）只要有 `error.code` 就原样抛出，`errorCode()` 取该 code，导致上游 raw message（如 "already has an active writer"）经 `toolErrorContent` 直接漏给模型，code 落为 `internal_error`。违反"不泄漏上游 raw"。计划：将非项目 `ERROR_CODES` 的上游错误映射为稳定领域码（建议新增 `thread_locked`），message 用固定文案。
    - 待办优先级：错误映射收敛 > 锁争用的可恢复提示（如 send 前探测锁并给出明确指引）。本轮实测细节见 docs/autonomous-runs/20260911-1340-ten-tool-e2e-and-interrupt.md。
